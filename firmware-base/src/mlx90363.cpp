#include "mlx90363.h"

#include <SPI.h>

#include "config.h"

namespace Mlx90363 {
namespace {

const SPISettings kSpi(HALL_SPI_HZ, MSBFIRST, SPI_MODE1);

const uint8_t kCrcTab[256] = {
    };

uint8_t computeCrc(const uint8_t* b) {
    uint8_t crc = 0xFF;
    for (int i = 0; i < 7; i++)
        crc = kCrcTab[b[i] ^ crc];
    return (uint8_t)(~crc);
}

void transfer(const uint8_t tx[8], uint8_t rx[8]) {
    SPI.beginTransaction(kSpi);
    digitalWrite(PIN_HALL_SS, LOW);
    delayMicroseconds(5);
    for (int i = 0; i < 8; i++)
        rx[i] = SPI.transfer(tx[i]);
    digitalWrite(PIN_HALL_SS, HIGH);
    SPI.endTransaction();
}

} // namespace

bool begin() {
    // Manual /SS on GP17. begin(false) keeps it GPIO — do NOT setCS to an illegal pin
    // (valid SPI0 CS: 1,5,17,21 only; setCS(22) panics and freezes the Pico).
    pinMode(PIN_HALL_SS, OUTPUT);
    digitalWrite(PIN_HALL_SS, HIGH);

    SPI.setRX(PIN_HALL_MISO);
    SPI.setTX(PIN_HALL_MOSI);
    SPI.setSCK(PIN_HALL_SCLK);
    SPI.setCS(PIN_HALL_SS);
    SPI.begin(false);

    delay(50);
    return true;
}

bool readAlphaDegrees(float& degreesOut) {
    const uint8_t get1[8] = {0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x13, 0xEA};
    const uint8_t nop[8] = {0x00, 0x00, 0xAA, 0xAA, 0x00, 0x00, 0xD0, 0xAB};
    uint8_t rx[8] = {0};

    transfer(get1, rx);
    delayMicroseconds(800);
    transfer(nop, rx);

    if (computeCrc(rx) != rx[7])
        return false;

    const uint16_t alpha = (uint16_t)(((rx[1] & 0x3F) << 8) | rx[0]);
    degreesOut = alpha * (360.0f / 16384.0f);
    return true;
}

} // namespace Mlx90363
